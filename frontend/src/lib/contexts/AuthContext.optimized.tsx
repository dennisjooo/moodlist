...
export function AuthProvider({ children }: AuthProviderProps) {
  ...
  const setCachedAuth = useCallback((user: User) => {
  ...
  const checkAuthStatus = useCallback(async (retryCount = 0, skipCache = false) => {
    try {
  ...
  }, [user, getCachedAuth, setCachedAuth, clearCachedAuth]);
  ...
  }, [getCachedAuth, clearCachedAuth, checkAuthStatus]);
  ...
  const value = useMemo<AuthContextType>(() => ({
    ...
    logout,
    refreshUser,
  }), [user, isLoading, isValidated, login, logout, refreshUser]);
