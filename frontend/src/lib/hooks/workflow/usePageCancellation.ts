...
interface UsePageCancellationProps {
    sessionId: string | null;
    stopWorkflow: () => void;
    clearError: () => void;
}

export function usePageCancellation({
    sessionId,
    stopWorkflow,
    clearError
}: UsePageCancellationProps) {
        ...
        try {
            ...
            if (sessionId) {
                try {
                    ...
                    logger.info('Workflow cancelled on backend', { component: 'usePageCancellation', sessionId });
                } catch (error) {
                    logger.error('Failed to cancel workflow on backend', error, { component: 'usePageCancellation', sessionId });
                    // Continue with local cleanup even if backend call fails
                }

                // Stop SSE/polling connections immediately to prevent further API calls
                logger.debug('Stopping SSE/polling connections for cancelled workflow', { component: 'usePageCancellation', sessionId });
