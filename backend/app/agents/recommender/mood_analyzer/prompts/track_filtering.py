...
def get_track_relevance_prompt(
    track_info: Dict[str, Any],
    mood_prompt: str,
    mood_analysis: Dict[str, Any]
) -> str:
    """Get prompt for determining if a track is culturally/linguistically relevant.

    Args:
        track_info: Track information including name, artists, genres
        mood_prompt: User's original mood prompt
        mood_analysis: Full mood analysis context

    Returns:
        Prompt string for track relevance assessment
    """
    ...
    return f"""Analyze whether this track matches the user's cultural/linguistic intent.
...
Task: Determine if this track is culturally and linguistically appropriate for the user's request.

Consider:
1. **Language/Region Match**: Does the track's language/region align with the mood request?
   - If user requested "French funk", Indonesian/Thai/Polish tracks are mismatches
   - If user requested "K-pop", Korean tracks are appropriate
   - If no specific region mentioned, Western/English tracks are generally safe

2. **Genre Coherence**: Does the musical style match the requested mood?
   - A Polish disco track doesn't fit "French funk"
   - An Indonesian pop track doesn't fit "nu-disco"

3. **Cultural Context**: Does the track's cultural origin fit the aesthetic?
   - French/European electronic music for "French funk"
   - Asian music only if explicitly requested

Respond in JSON format:
{{
  "is_relevant": true,
  "confidence": 0.95,
  "reasoning": "Track matches the French funk aesthetic with European electronic production",
  "language_region": "French/European",
  "should_include": true
}}

Set `should_include` to false if there's a clear cultural/linguistic mismatch."""
