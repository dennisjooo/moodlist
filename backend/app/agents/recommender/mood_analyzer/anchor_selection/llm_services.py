...
class LLMServices:
    ...
    async def extract_user_mentioned_tracks(
        self, mood_prompt: str, artist_recommendations: List[str]
    ) -> List[Tuple[str, str]]:
    ...
    def simple_extract_mentioned_tracks(
        ...
        if "especially" in mood_prompt.lower():
            ...
            if len(parts) > 1:
                ...
                primary_artist = (
                    artist_recommendations[0] if artist_recommendations else ""
                )
    ...
    async def finalize_selection(
        ...
        for candidate in scored_candidates:
            if candidate.get("anchor_type") == "user" or candidate.get(
                "user_mentioned"
            ):
        ...
        for candidate in user_mentioned:
            ...
            track["llm_reasoning"] = candidate.get(
                "llm_reasoning", "User explicitly mentioned this track"
            )
        ...
        if remaining_slots == 0:
            logger.info(
                f"Target count ({target_count}) reached with user-mentioned tracks only"
            )
            return selected_tracks, selected_ids

        if not other_candidates:
            logger.info(
                f"No additional candidates to fill remaining {remaining_slots} slots"
            )
            return selected_tracks, selected_ids
        ...
        quality_candidates = [
            c for c in other_candidates if c.get("llm_score", 0) >= quality_threshold
        ]
        ...
        if not other_candidates:
            logger.warning(
                f"No quality candidates remain after filtering (threshold={quality_threshold})"
            )
            return selected_tracks, selected_ids
        ...
        if not self.llm:
            ...
            logger.info(
                f"Added {len(additional)} additional tracks (no LLM, score-based)"
            )
            return selected_tracks, selected_ids
        ...
        try:
            ...
            logger.info(
                f"Added {len(additional)} additional tracks (LLM failed, score-based fallback)"
            )
            return selected_tracks, selected_ids
    ...
    async def batch_validate_artists(
        ...
        try:
            prompt = get_batch_artist_validation_prompt(
                artists, mood_prompt, mood_analysis
            )
            response = await self.llm.ainvoke([{"role": "user", "content": prompt}])
            result = LLMResponseParser.extract_json_from_response(response)
            ...
            logger.info(
                f"Batch artist validation: kept {len(validated)}/{len(artists)} artists"
            )
            return validated
    ...
    async def filter_tracks_by_relevance(
        ...
        try:
            ...
            prompt = get_batch_track_filter_prompt(
                tracks_for_llm, mood_prompt, mood_analysis
            )
            response = await self.llm.ainvoke([{"role": "user", "content": prompt}])
            result = LLMResponseParser.extract_json_from_response(response)
            ...
            for filter_info in filtered_out:
                ...
                if 0 <= track_idx < len(tracks_for_llm):
                    ...
                    is_protected = track_data.get(
                        "user_mentioned", False
                    ) or track_data.get("protected", False)
            ...
            filtered_tracks = [
                tracks[i] for i in range(len(tracks)) if i in relevant_indices
            ]
