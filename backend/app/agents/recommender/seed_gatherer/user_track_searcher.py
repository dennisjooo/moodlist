...
class UserTrackSearcher:
    ...
    async def _search_single_track(
        ...
        """Search for a single user-mentioned track.
        ...
        Returns:
            Found track dict or None if not found
        """
    ...
    async def search_user_mentioned_tracks(
        ...
        """Search for tracks explicitly mentioned by the user (parallelized).
        ...
        logger.info(f"Searching for {len(user_mentioned_tracks)} user-mentioned tracks in parallel")

        # Search all tracks in parallel
        search_tasks = [
            self._search_single_track(track_info, access_token)
            for track_info in user_mentioned_tracks
        ]
        search_results = await asyncio.gather(*search_tasks)

        # Filter out None results and build return lists
        found_tracks = [track for track in search_results if track is not None]
