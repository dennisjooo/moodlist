...
class TrackRecommendationFactory:
    ...
    def from_spotify_track(
        ...
        try:
            ...
            audio_features = TrackRecommendationFactory._extract_audio_features(track_data)
            ...
            logger.warning(f"Failed to create TrackRecommendation from Spotify data: {e}")
            return None
    ...
    def from_reccobeat_response(
        ...
        try:
            ...
            reasoning = (
                ...
                f"Recommended by RecoBeat from {source}"
            )
            ...
            logger.warning(f"Failed to create TrackRecommendation from RecoBeat data: {e}")
            return None
    ...
    def from_seed_based_generation(
        ...
        reasoning = (
            reasoning or
            f"Mood-based recommendation using seeds: {', '.join(seed_tracks[:3])}"
        )
    ...
    def _extract_audio_features(track_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        audio_features = (
            track_data.get("audio_features") or
            track_data.get("features") or
            {}
        )
        ...
        for track_data in track_data_list:
            try:
                if factory_method == "spotify":
                    rec = TrackRecommendationFactory.from_spotify_track(track_data, **kwargs)
                elif factory_method == "reccobeat":
                    rec = TrackRecommendationFactory.from_reccobeat_response(track_data, **kwargs)
                elif factory_method == "artist":
                    rec = TrackRecommendationFactory.from_artist_top_track(track_data, **kwargs)
                else:
