...
class LimitsConfig:
    ...
    genre_anchor_search_limit: int = (
        5  # How many genres to fan out when gathering anchors
    )
    ...
    user_track_extraction_limit: int = (
        5  # Max number of track mentions to try resolving
    )
    user_track_search_results_limit: int = (
        5  # How many Spotify results to inspect per mentioned track
    )
    artist_anchor_processing_limit: int = (
        5  # Max artists to process for anchor building per request
    )
    ...
    artist_search_limit: int = (
        3  # Number of Spotify artist search results to inspect per query
    )
    ...
    fallback_artist_search_limit: int = (
        12  # Spotify limit when probing fallback keywords
    )
    llm_batch_validation_trigger: int = (
        10  # Min artist count before running batch LLM filtering
    )
    llm_minimum_filtered_artists: int = (
        5  # Minimum acceptable LLM output before falling back
    )
    ...
    heuristic_pruning_min_artists: int = (
        15  # Skip pruning when artist pool is already small
    )
    heuristic_min_artist_popularity: int = (
        15  # Minimum popularity before we discard an artist
    )
    ...
    genre_expansion_core_limit: int = (
        10  # Core artists considered when expanding via genre search
    )
    ...
    genre_expansion_min_popularity: int = (
        20  # Minimum popularity for expansion candidates
    )

    # Diversity balancing controls
    popularity_tier_mainstream_threshold: int = (
        70  # Popularity > threshold => mainstream
    )
    popularity_tier_mid_threshold: int = 40  # Popularity >= threshold => mid-tier
    popularity_tier_mainstream_ratio: float = 0.3  # 30% mainstream
    popularity_tier_mid_ratio: float = 0.5  # 50% mid-tier
    popularity_tier_niche_ratio: float = 0.2  # 20% niche
