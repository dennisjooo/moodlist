...
class PhaseAssigner:
    ...
    def _calculate_phase_distribution(
        self, strategy: Dict[str, Any], track_count: int
    ) -> Dict[str, int]:
        ...
        phase_distribution = strategy.get(
            "phase_distribution",
            {"opening": 2, "build": 5, "mid": 8, "high": 7, "descent": 4, "closure": 2},
        )
        ...
        adjusted_distribution = self._scale_distribution(
            normalized_distribution, track_count
        )

        # Ensure exact track count
        adjusted_distribution = self._adjust_to_exact_count(
            adjusted_distribution, track_count
        )

        logger.info(
            f"Phase distribution for {track_count} tracks: {adjusted_distribution}"
        )

        return adjusted_distribution

    def _normalize_phase_names(
        ...
        invalid_phases = {
            k: v for k, v in phase_distribution.items() if k not in all_phases
        }
        ...
        if invalid_phases:
            ...
            if extra_tracks > 0:
                logger.info(
                    f"Redistributing {extra_tracks} tracks from custom phases to 'mid'"
                )
    ...
    def _scale_distribution(
        self, distribution: Dict[str, int], target_count: int
    ) -> Dict[str, int]:
    ...
    def _assign_using_greedy_best_fit(
        self, track_phase_scores: List[tuple], adjusted_distribution: Dict[str, int]
    ) -> Dict[str, List[TrackRecommendation]]:
