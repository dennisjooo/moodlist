...
class RecoBeatIDRegistry:
    ...
    VALIDATED_ID_TTL = (
        86400 * 180
    )  # 180 days - validated IDs are immutable (increased from 60 days)
    ...
    async def mark_validated(cls, spotify_id: str, reccobeat_id: str) -> None:
        ...
        try:
            ...
            await cache_manager.cache.set(
                reverse_key, reverse_data, ttl=cls.REVERSE_ID_TTL
            )
            logger.debug(
    ...
    @classmethod
    async def bulk_check_missing(
        cls, spotify_ids: list[str]
