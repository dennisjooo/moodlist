...
class CoverImageGenerator:
    ...
    def generate_cover_base64(
        ...
        tertiary_color: str,
        style: Literal[
            "diagonal", "radial", "mesh", "waves", "minimal", "modern"
        ] = "modern",
    ) -> str:
        ...
        """
        jpeg_bytes = self.generate_cover(
            primary_color, secondary_color, tertiary_color, style
        )
            ...
            for x in range(self.size):
                ...
                if factor < 0.33:
                    ...
                    color = self._interpolate_color(
                        colors[1], colors[2], (factor - 0.33) * 3
                    )
                else:
                    color = self._interpolate_color(
                        colors[2], colors[0], (factor - 0.66) * 3
                    )
    ...
    def _generate_minimal(self, colors: list) -> Image.Image:
        ...
        draw.ellipse(
            [
                (self.size - circle_size, self.size - circle_size),
        ...
        draw.ellipse([(-100, -100), (small_circle, small_circle)], fill=colors[2])
    ...
    def _image_to_jpeg_bytes(self, image: Image.Image, quality: int = 95) -> bytes:
        ...
        if size_kb > 256:
            logger.warning(
                f"Image size {size_kb:.1f}KB exceeds Spotify limit, reducing quality"
            )
        ...
        logger.info(
            f"Generated cover image: {len(jpeg_bytes)} bytes ({len(jpeg_bytes) / 1024:.1f}KB)"
        )
        return jpeg_bytes
